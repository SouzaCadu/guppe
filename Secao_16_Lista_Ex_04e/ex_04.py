"""
4) Crie uma classe Televisão e uma classe ControleRemoto que pode controlar o volume e trocar os
   canais da televisão.
...
class Televisao:
    ...
    def set_volume(self, novo_volume):
        ...
        de instância 'volume'"""
        try:
            if int(novo_volume) < 0 or int(novo_volume) > 100:
                raise ValueError
            else:
    ...
    def set_quantidade_canais(self, nova_quantidade_canais):
        try:
            if not int(nova_quantidade_canais) <= 0 and int(nova_quantidade_canais) >= self.__canal:
