...
def cria_arquivos(secao, cod_aula, total_aulas, descr):
    for i in range(total_aulas):
        cod_aula = cod_aula + i
        arquivo = os.path.join(os.getcwd(), secao + "_" + str(cod_aula) + "_" + descr[i] + ".py")
        print(arquivo)
        try:
...
diretorio = str(input("Informe o diretorio onde serão criados os arquivos no formato 'Seção_nn': "))
os.mkdir(os.path.join(os.getcwd(), diretorio))
os.chdir(os.path.join(os.getcwd(), diretorio))
secao = str(input("Informe a seção do curso no formato 'Seção_nn': "))
